    ...
    <section className="px-3 py-8 w-full flex flex-col items-center text-center">
      <div className="w-full max-w-[390px] flex flex-col items-center space-y-3">
        ...
        <div className="w-full relative rounded-3xl bg-gradient-to-b from-[#3A1008] via-[#260B06] to-[#1A0805] border border-[#D4AF5A]/60 shadow-[0_12px_28px_rgba(0,0,0,0.85),0_0_20px_rgba(184,134,50,0.18)] p-5 flex flex-col items-center overflow-hidden">
          ...
          <h2 className="relative z-10 font-cinzel text-xs sm:text-sm font-bold tracking-widest text-[#FFF1D0] uppercase mb-1">
            TO GANESH CHATURTHI
          </h2>
          <p className="relative z-10 font-asar text-xs text-[#F4E5C3]/80 font-semibold mb-4">
            14 September 2026 • 7:30 PM
          </p>
          ...
          {timeLeft.isComplete ? (
            <div className="relative z-10 py-3 px-5 rounded-2xl bg-[#4A1009]/80 border border-[#D4AF5A]/60 flex flex-col items-center justify-center gap-1 shadow-inner">
              <span className="font-devanagari text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-[#FFF1D0] via-[#E7C56A] to-[#D4AF5A] drop-shadow">
                गणपति बप्पा मोरया
              </span>
              <span className="font-asar text-xs text-[#FFF1D0]">
                The Divine Festivities Have Begun!
              </span>
            </div>
          ) : (
            /* ONE SINGLE HORIZONTAL ROW: DAYS | HOURS | MINUTES | SECONDS (No 2x2 wrapping!) */
            <div className="relative z-10 w-full flex items-center justify-between gap-1.5 sm:gap-2 px-1">
              {timeUnits.map((unit) => (
                <div key={unit.label} className="flex-1 flex flex-col items-center justify-center">
                  {/* Compact Elegant Gold Ornamental Circle */}
                  <div className="relative w-15 h-15 sm:w-17 sm:h-17 rounded-full border border-[#D4AF5A] bg-gradient-to-b from-[#260B06] via-[#1A0805] to-[#120604] shadow-[inset_0_2px_4px_rgba(255,241,208,0.25),0_6px_14px_rgba(0,0,0,0.8)] flex flex-col items-center justify-center">
                    <div className="absolute inset-1 rounded-full border border-dashed border-[#D4AF5A]/40 pointer-events-none" />

                    <span className="font-yatra text-xl sm:text-2xl font-black text-transparent bg-clip-text bg-gradient-to-b from-[#FFF1D0] via-[#E7C56A] to-[#D4AF5A] drop-shadow leading-none">
                      {String(unit.value).padStart(2, "0")}
                    </span>
                  </div>
                  <span className="font-cinzel text-[8px] sm:text-[9px] font-bold uppercase tracking-wider text-[#F4E5C3]/90 mt-1.5">
                    {unit.label}
                  </span>
                </div>
              ))}
